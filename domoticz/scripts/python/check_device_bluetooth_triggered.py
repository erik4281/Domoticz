...
def domoticzstatus ():
  ...
  status = 0
  switchfound = False
  log (datetime.datetime.now().strftime("%H:%M:%S") + "- status part 1")

  if json_object["status"] == "OK":
    for i, v in enumerate(json_object["result"]):
      if json_object["result"][i]["idx"] == switchid and "Lighting" in json_object["result"][i]["Type"] :
        switchfound = True
        log (datetime.datetime.now().strftime("%H:%M:%S") + "- status part 2")
...
def domoticztrigger ():
  ...
  status = 0
  switchfound = False
  log (datetime.datetime.now().strftime("%H:%M:%S") + "- trigger part 1")
 
  if json_object["status"] == "OK":
    for i, v in enumerate(json_object["result"]):
      if json_object["result"][i]["idx"] == triggerid and "Lighting" in json_object["result"][i]["Type"] :
        switchfound = True
        log (datetime.datetime.now().strftime("%H:%M:%S") + "- trigger part 2")
        if json_object["result"][i]["Status"] == "On": 
          status = 1
        if json_object["result"][i]["Status"] == "Off": 
          status = 0
